...
public class TcpTest extends IoChannelTesterBase<MessageBuffer> {
    ...
    private void test(TcpClientChannel client1, TcpClientChannel client2, ChannelFactory channelFactory)
            throws Exception {
        ...
        try (Closer closer = Closer.create()) {
            ...
            // connect client1 first so that we can get it's id in the server connection
            ...
            for (int i = 0; i < 5; i++) {
                MessageBuffer mb = new MessageBuffer(streamIo.prePendLength(new String("Hello World " + i).getBytes()));
            ...
            for (int i = 5; i < 10; i++) {
                server.send(new MessageBuffer(streamIo.prePendLength(new String("Hello World " + i).getBytes())));
            }
    ...
    public void test() throws Exception {
        ...
        TcpClientChannel client1 = TcpClientChannel.createChannel("client1", new SimpleStreamIo(), localAddress0,
                remoteAddress);

        client1.addMessageListener(queue1::add);

        TcpClientChannel client2 = TcpClientChannel.createChannel("client2", queue2::add, new SimpleStreamIo(), null,
                remoteAddress);
    ...
    public void testClientSendingToServer() throws Exception {
        ...
        TcpServerChannel server = TcpServerChannel.createChannel("server", queue::add, streamIo, address5001);
        TcpClientChannel client = TcpClientChannel.createOutputOnlyChannel("client", streamIo, localAddress0,
                address5001);
    ...
    public void testOffNominal() throws Exception {
        InetSocketAddress address5001 = new InetSocketAddress(InetAddress.getLocalHost(), 5001);
        TcpServerChannel server = TcpServerChannel.createOutputOnly("server", new SimpleStreamIo(), address5001);
        server.connect();
